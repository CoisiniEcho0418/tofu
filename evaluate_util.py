...
from src.data_module import TextDatasetQA, custom_data_collator, get_batch_loss, custom_data_collator_with_indices
...
def eval_perturbation_ratio(eval_dataloader, perturb_dataloader, model):
    ...
    for batch, perturb_batch in tqdm(zip(eval_dataloader, perturb_dataloader)):
        ...
        perturb_loss_per_token = dict(
            zip(indices.cpu().numpy().tolist(), perturb_loss_per_token.detach().to(torch.float).cpu().numpy().tolist())
        )
        gt_loss_per_token = dict(
            zip(indices.cpu().numpy().tolist(), gt_loss_per_token.detach().to(torch.float).cpu().numpy().tolist())
        )
        truth_ratio = dict(
            zip(indices.cpu().numpy().tolist(), truth_ratio.detach().to(torch.float).cpu().numpy().tolist())
        )
        gt_loss = dict(zip(indices.cpu().numpy().tolist(), gt_loss.detach().to(torch.float).cpu().numpy().tolist()))
        perturb_loss = dict(
            zip(indices.cpu().numpy().tolist(), perturb_loss.detach().to(torch.float).cpu().numpy().tolist())
        )
...
def get_all_evals(
    cfg, model, tokenizer, eval_task, eval_dataloader, base_eval_dataloader, perturb_dataloader, normalize_gt=False
):
    eval_logs = {}
    ...
    for batch in tqdm(eval_dataloader):
        ...
        eval_logs["avg_gt_loss"].update(
            dict(zip(indices.cpu().numpy().tolist(), gt_loss_per_token.detach().to(torch.float).cpu().numpy().tolist()))
        )
        eval_logs["gt_loss"].update(
            dict(zip(indices.cpu().numpy().tolist(), gt_loss.detach().to(torch.float).cpu().numpy().tolist()))
        )
...
def main(cfg):
    ...
    # TODO: 修改 cfg
    cfg.split = "forget01_perturbed"
    unlearned_dir_name = "tofu_idk_result"
    unlearned_file_name = "edit_1e-05_forget01_1_wd0.01_bs40"

    # "locuslab_tofu_ft_phi/models--locuslab--tofu_ft_phi-1.5/snapshots/c34da369771ff53cf722344bc912fd9913f67da7"
    cfg.model_path = f"/home/wxy/wxy_workspace/LLM_unlearn/tofu-main/{unlearned_dir_name}/{unlearned_file_name}"
    cfg.save_dir = (
        (
            f"/home/wxy/wxy_workspace/LLM_unlearn/tofu-main/{unlearned_dir_name}/{unlearned_file_name}/eval_results/ds_size{cfg.ds_size}"
        )
        if "locuslab_tofu_ft_phi" not in cfg.model_path
        else f"/home/wxy/wxy_workspace/LLM_unlearn/tofu-main/{unlearned_dir_name}/locuslab_tofu_ft_phi/{cfg.split}/eval_results/ds_size{cfg.ds_size}"
    )
